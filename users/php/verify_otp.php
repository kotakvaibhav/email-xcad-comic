...
class verify_otp
{
    ...
    public function send_mail_fun($user_mail){
        ...
        $this->query = $this->db->prepare('SELECT count FROM user_data WHERE email=? ');
        $this->query->bind_param('s',$_POST['email']);
        $this->query->execute();
        $this->query->store_result();
        $this->query->bind_result($this->count);
        $this->query->fetch();
        if($this->count == null) {
            if(isset($_POST['status']) && $_POST['status'] == 'unsubscribe.php'){	
                $this->message = '	
                <body style=\'background-color:rgb(238,238,238);padding-top:10px;padding-bottom:10px;text-align:center;\'>	
                    <div style=\'width:50%;margin:0 auto;background-color:rgb(248,248,248);padding:10px\'>	
                        <h3>Hello User</h3>	
                        <h5>You have successfully unsubscribe for the XKCD Comics email services.</h5>	
                    </div>	
                </body>	
            ';        	
            }
            ...
            try {
                ...
                echo $this->message; echo 'Email Verified';
        
                if($mail->send()){                    
                    if($this->count == null){
                        $this->count = 1;
                        $this->query = $this->db->prepare('UPDATE user_data SET count=? WHERE email=?');
                        $this->query->bind_param('is',$this->count,$_POST['email']);
                        $this->query->execute();
                    }                                       
                }
            ...
            if(isset($_POST['status']) && $_POST['status'] == 'unsubscribe.php') { 	
                    ...
                    $this->query = $this->db->prepare('DELETE FROM user_data WHERE otp=? AND email=?');	
                    $this->query->bind_param('is',$_POST['otp'],$_POST['email']);	
                    $this->query->execute();	
